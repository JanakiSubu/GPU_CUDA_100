// Compile with: nvcc -o fcnet fcnet.cu -lcudnn -lcublas -lcurand
...
int main() {
    ...
    // Create tensor descriptors for all layers
    ...
    // Configure tensor shapes (NCHW format with height=1, width=1)
    ...
    // Define filter descriptors for fully-connected weights (1x1 conv)
    ...
    // Set up ReLU activation descriptor
    ...
    // Allocate input/output tensors and labels on GPU
    ...
    // Initialize weights and zero the biases
    ...
    for (int epoch = 0; epoch < epochs; ++epoch) {
        ...
        // Free intermediate layer outputs
        CHECK_CUDA(cudaFree(d_hidden1));
        CHECK_CUDA(cudaFree(d_hidden2));

        // Print output of final epoch for verification
        if (epoch == epochs - 1) {
